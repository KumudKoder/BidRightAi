  ...
  try {
    ...
    if (req.file) {
      ...
      try {
        ...
        console.log("Calling pdfParse, type:", typeof pdfParse);
        ...
        return res
          .status(500)
          .json({ error: "PDF parsing failed", details: pdfError.message });
      }
    } else if (req.body.text) {
      // This must match "text" in your swagger.json
      textToAnalyze = req.body.text;
      console.log("Using text from body, length:", textToAnalyze.length);
    } else {
...
Constraint: Output ONLY raw JSON. No conversational filler.`;
    ...
    console.log("Token received, calling IBM API...");
